...
public class SocketConnectionHandler implements Runnable
{
    ...
    public boolean setSocketStreamReaderWriter()
    {
        ...
        try
        {
            ...
            // receive the client certificate and validate it
            ...
            // Server keystore with symmetric keys and load the secret key
            ...
            // convert our secret key to a string and then to a byte array which can be encrypted
            String secKeyToString = Base64.getEncoder().encodeToString(secKey.getEncoded());
            byte[] toEncrypt = secKeyToString.getBytes();
            byte[] encryptedBytes = cipher.doFinal(toEncrypt);

            System.out.println("Message symkey sent: " + Helper.byteArrayToHex(encryptedBytes));
    ...
    public boolean verify (String hashPart, String msgPart, SecretKeySpec symKey) throws GeneralSecurityException {
        byte[] msgBytes = hexToByteArray(msgPart);
        byte[] newHash = calculateHmac(symKey, msgBytes);
        byte[] oldHash = hexToByteArray(hashPart);
        return Arrays.equals(newHash, oldHash);
    }

    /**
     * Append the hash and IV to the message which is to be sent. This creates the final product to be sent over the socket
     */
